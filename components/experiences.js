    ...
    <Stack spacing="12">
      {experiences.map(({ fields, sys }) => {
        ...
        const endDate = fields.endDate
          ? format(new Date(fields.endDate), "MMM, yyyy")
          : "Current";

        return (
          <Box key={sys.id}>
            <Text fontSize="sm" color="GrayText" display="flex" alignItems="center">
              {startDate} - {endDate} <Badge fontSize="xx-small" ml="2" colorScheme={fields.type == "Full Time" ? 'whatsapp' : 'blue'}>{fields.type}</Badge>
            </Text>
            <Text as="h4" display="flex" flexDirection={{ base: "column", sm: "row" }} fontSize="xl">
              {fields.company}{" "}
